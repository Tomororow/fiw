			...
			create: function (opt) {
				...
				fn = {
					_normal: function () {
						...
						options.popObj.find("div.popup-box").css({ "left": (($(document).width() - options.popObj.find("div.popup-box").outerWidth()) / 2.8) + "px", "top": (($(document).height() - options.popObj.find("div.popup-box").outerHeight()) / 2.7) + "px" });
					},
					_ajax: function () {
						$.ajax({
							type: "POST",
