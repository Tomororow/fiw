	...
	$.ajax({
		...
		success:function(data){
			if(data == "success"){
				...
				ztreeFun($("#ztree"), "sysWaterArea/getAllArea.do?isSearchDevice=true&showOnLineDevice=false", intelligentDealList);
			}else if(data == "failed"){
				iden_flag = data;
				// 水管区域和行政区域同时配置  则按照行政区域查询
				ztreeFun($("#ztree"), "sysArea/getAllArea.do?isSearchDevice=false&showOnLineDevice=false", intelligentDealList);
			}
		}
	});
}
 ...
 * 机井设备信息查询
 */
function intelligentDealList(type) {
	var params = {};
	debugger
 ...
 * 设备告警信息导出
 */
function alarmThreeVoltageReport(){
	debugger
	var frt = $("#warnList").val();
	if(frt==0){
		confirmMsg("请先查询相关数据...");
	}else{
		window.location.href="warning_Record/warn_excel.do?handleType=1";
	}
}
...
function loadIntelligentDealInfo(params, flag) {
	...
	if(iden_flag == "success"){
		...
		$.post("warning_Record/warnNoClose.do",params, function(data){
			$("#intelligentDealContent").html(data);
