...
<script type="text/javascript" src="${ctx}/content/skin/js/intelligentDealReport.js"></script>
			   ...
			   <span style="margin-left:10px;">告警类型：</span>
		 	        <select class="pop-select" id="alarmType" name="alarmType" style="display:inline-block; width:118px;">
			   			...
			   			<c:forEach var="item" items="${TypeList}">
			   				<option value="${item.abnormalcode}">${item.abnormaltype}</option>
			   			</c:forEach>
			   		</select>
			    <input type="button" class="btn-search" value="查 询" onclick="intelligentDealList(1)">
				<!-- <input type="button" class="btn-reset"  value="重置" onclick="intelligentDealReset()"> -->
				<input type="button" class="btn-export-report" value="信息导出" onclick="alarmThreeVoltageReport()">
			</div>
		</div>
	</div>
	<div id="intelligentDealContent"></div>
</div>