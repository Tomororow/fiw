...
<div class="right_user" style="left:15px; top:50px;">
	...
	<div style="width:20%; float:left;">
		...
		<p style="font-size:14px; font-weight:bold;">查询年份：${startTime} 年</p>
	</div>
				...
				<c:forEach var="item" items="${uwdList}" varStatus="vs">
					<tr class="${vs.index%2==0?'singular':'double'}">
						...
						<td>
							...
							<c:if test="${item.createTime != null || item.createTime != ''}">
								<fmt:formatDate value="${item.createTime}" pattern="yyyy" />
							</c:if>
						</td>
						<td>${item.useWaterOfYear}</td>
					</tr>
				</c:forEach>
			</c:if>
		</tbody>
...
var sumyearwaterList = [];
	$(function() {
		...
		monthList = "${monthList}";
		sumyearwaterList = "${sumyearwaterList}";
		if(sumyearwaterList!=""&&monthList!=""){
			sumyearwaterList = JSON.parse(sumyearwaterList);
			monthList = JSON.parse(monthList);
		}
		  for(var i=0;i<monthList.length;i++){
			  debugger
				  len.push(monthList[i]);
				  lendata.push({value:sumyearwaterList[i],name:monthList[i]+'月'});
				  seleted[monthList[i]]=i<6;
		 } 
	});
