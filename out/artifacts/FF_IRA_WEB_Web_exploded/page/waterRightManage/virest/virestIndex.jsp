		...
		<div class="operate">
			<ul>
				<c:if test="${sysWaterArea.waterAreaLevel<2}">
					<li class="add" id="show" onclick="addVirest(this)" style="line-height:20px;" data-src="waterRightManage/addVirestPage.do?isAppendWater=0"><font style="font-size: 15px;">新增配水计划</font></li>
					<li class="del" onclick="delPlan(this)" style="line-height:20px;" data-fl="2"><font style="font-size: 15px;">删除</font></li>
				</c:if>
				<%-- <c:if test="${sysWaterArea.waterAreaLevel<2}">
					<li class="addAppendWater" id="show" style="line-height:20px;" onclick="addVirest(this)" data-src="waterRightManage/addVirestPage.do?isAppendWater=1"><font style="font-size: 15px;">阶梯性追加水量</font></li>
				</c:if> --%>
			</ul>
		</div>
		<div class="device" id="baseDistWaterPlanList">
			<table cellspacing='0' cellpadding='3' class='list-table'>
				...
				<tbody>
					...
					<c:if test="${!empty virestList}">
						<c:forEach var="item" items="${virestList}" varStatus="vs">
							<tr class="${vs.index%2==0?'singular':'double'}">
								<td><input type="checkbox" id="${item.id}" name="${item.id}"  value="${item.id}" /></td>
								<td>${item.waterAreaName}</td>
								<td><a onclick="showDistWaterDeviceInfo('${item.id}',${item.distType})" style="cursor: pointer;">按绿化井配水方式配水</a></td>
								<td>${item.distYear}</td>
								...
								<td width="30">
		                            <ul style="width:60px; padding-left:15px;">
		                                <li class="del" title="删除" onclick="delPlan(this)" data-fl="2" data-id='${item.id}'></li>
		                            </ul>
	                          	</td>
							</tr>
						</c:forEach>
