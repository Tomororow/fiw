		...
		<div class="operate">
			<ul>
				<c:if test="${sysWaterArea.waterAreaLevel<2}">
					<li class="add" id="show" onclick="addIndustry(this)" style="line-height:20px;" data-src="waterRightManage/addIndustryPage.do?isAppendWater=0"><font style="font-size: 15px;">新增配水计划</font></li>
					<li class="del" onclick="delPlan(this)" style="line-height:20px;" data-fl="2"><font style="font-size: 15px;">删除</font></li>
				</c:if>
				<%-- <c:if test="${sysWaterArea.waterAreaLevel<2}">
					<li class="addAppendWater" id="show" style="line-height:20px;" onclick="addIndustry(this)" data-src="waterRightManage/addIndustryPage.do?isAppendWater=1"><font style="font-size: 15px;">阶梯性追加水量</font></li>
				</c:if> --%>
			</ul>
		</div>
					...
					<c:if test="${!empty industryList}">
						<c:forEach var="item" items="${industryList}" varStatus="vs">
							<tr class="${vs.index%2==0?'singular':'double'}">
								...
								<td><a onclick="showDistWaterDeviceInfo('${item.id}',${item.distType})" style="cursor: pointer;">按年核定水量配水</a></td>
								<td>${item.distYear}</td>
